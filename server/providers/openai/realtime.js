...
// SECURITY: loopback-only guard for the token minting endpoint. This endpoint
// uses the server's secret OPENAI_API_KEY to mint ephemeral session tokens —
// any caller who can reach it gets a working OpenAI credential at the owner's
// expense. The key-setup endpoints already enforce this (admitKeySetupRequest),
// but the token endpoint — which actually SPENDS the key — had none. With
// HOST=0.0.0.0 every device on the LAN could mint tokens without restriction.
...
function createRealtimeTokenHandler({
  ...
  return async (req, res) => {
    ...
    // SECURITY: reject non-loopback callers. With HOST=0.0.0.0 the server is
    // reachable from the LAN; without this check any device on the network
    // could mint OpenAI tokens and drain the owner's billing account.
    const remote = String(req.socket?.remoteAddress || '');
    if (!LOOPBACK_ADDRESSES.has(remote)) {
      res.statusCode = 403;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'Token minting is restricted to the local machine' }));
      return;
    }
