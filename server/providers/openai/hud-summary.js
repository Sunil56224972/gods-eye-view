...
// SECURITY: same loopback guard used by the realtime token endpoint. Every
// call to hud-summary spends OpenAI tokens using the server's secret key.
...
async function handleHudSummary(req, res) {
  ...
  // SECURITY: reject non-loopback callers before touching the API key.
  const remote = String(req.socket?.remoteAddress || '');
  if (!LOOPBACK_ADDRESSES.has(remote)) {
    res.statusCode = 403;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ error: 'HUD summary is restricted to the local machine' }));
    return;
  }
